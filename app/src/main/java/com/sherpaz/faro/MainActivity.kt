...
class MainActivity : AppCompatActivity() {
    ...
    override fun onResume() {
        ...
        tvO.text = if (Settings.canDrawOverlays(this)) "Permiso flotante: ACTIVO" else "Permiso flotante: INACTIVO"
        val svc = "${packageName}/${UberAccessibilityService::class.java.canonicalName}"
        val enabled = Settings.Secure.getString(contentResolver, Settings.Secure.ENABLED_ACCESSIBILITY_SERVICES) ?: ""
        tvA.text = if (enabled.contains(svc)) "Accesibilidad: ACTIVA" else "Accesibilidad: INACTIVA"
    }
}
